 ...
 "cells": [
  {
   ...
   "source": [
    ...
    "This Jupyter Notebook acts as the end-to-end orchestration controller for the final thesis evaluation, executing:\n",
    "1. **Comparative Benchmark (Ablation Study):** Global evaluation of three frozen model versions (Legacy, Base, ACP) against the AASIST3 detector.\n",
  ...
  },
  {
   ...
   "source": [
    ...
    "except ImportError:\n",
    "    logger.info(\"Not running in Google Colab. Skipping Drive mount.\")\n",
    "\n",
    "# Ensure model sub-folders for sample storage exist\n",
    "for folder in [\"legacy_samples\", \"base_samples\", \"acp_samples\"]:\n",
    ...
    "                \n",
    "        report_content = f\"\"\"# Master's Thesis Evaluation: Final Results Summary (Time-Budget Terminated)\n",
  ...
  },
  {
   ...
   "source": [
    ...
    "Loops through the three model checkpoints (Legacy, Base, and ACP). For each model, it checks the status manifest to skip already generated sets. It calls `should_continue` on the `TimeBudgetManager` (using rolling average task duration) before running the generation.  \n",
  ...
  },
  {
   ...
   "metadata": {},
   "source": [
    "## Cell 5: Stage 4: Visual Reporting & Thesis Summary\n",
    "Generates final thesis figures and saves them in the outputs folder. Then, compiles a comprehensive thesis report containing global benchmarks and cluster breakdowns, and exports it to `outputs/evaluation/final_results.md`."
   ]
  },
  {
   "cell_type": "code",
   ...
   "source": [
    ...
    "    table3 += f\"| Resilient (Robust) | {sum(res_mask)} | {mean_asr_res:.2f}% | {mean_eer_res:.2f}% | Hardened manifolds; high resistance to adversarial signal deformation. |\\n\"\n",
    "    \n",
    "    thesis_report = f\"\"\"# Master\'s Thesis Evaluation: Final Results Summary\n",
    ...
    "        \n",
    "    print(\"\\n\" + \"=\"*80)\n",
    "    print(\"THESIS RESULTS REPORT SUMMARY (EXPORTED TO \" + report_path + \")\")\n",
    "    print(\"=\"*80)\n",
    "    print(thesis_report)\n",
    "    print(\"=\"*80)\n",
    "else:\n",
...
# Ensure directory exists and write notebook
os.makedirs("notebooks", exist_ok=True)
notebook_path = "notebooks/06_Final_evaluation.ipynb"

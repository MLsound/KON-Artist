...
def generator_from_ds(shuffled_ds):
    ...
    for sample in shuffled_ds:
        # ASVspoof 2019 LA Raw: 0 is Bonafide, 1 is Spoof
        # KON-Artist Mapping: 1 is Bonafide, 0 is Spoof (Aligned with AASIST3 output indices)
        original_label = sample["key"]
        mapped_label = 1 - original_label # Flip: 0 -> 1 (Bonafide), 1 -> 0 (Spoof)

        # MANDATORY: Only yield spoofed signals (mapped_label == 0).
        if mapped_label == 1:
            continue 
...
def eval_generator(shuffled_ds):
    ...
    for sample in shuffled_ds:
        ...
        # ASVspoof 2019 LA Raw: 0 is Bonafide, 1 is Spoof
        # KON-Artist Mapping: 1 is Bonafide, 0 is Spoof
        original_label = sample["key"]
        mapped_label = 1 - original_label # Flip: 0 -> 1 (Bonafide), 1 -> 0 (Spoof)

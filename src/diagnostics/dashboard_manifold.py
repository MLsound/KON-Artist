...
def load_data(winners_path):
    ...
    if "cluster" in winners_df.columns:
        winners_df["cluster"] = pd.to_numeric(winners_df["cluster"], errors="coerce").fillna(0).astype(int)
    return winners_df
...
def interpolate_global_surface(df, x_col, y_col, z_col, grid_size=60):
    ...
    """
    # Use unique grouping columns
    group_cols = list(dict.fromkeys([x_col, y_col]))
    # Remove any columns already used in grouping from the aggregation columns list
    columns_to_agg = [c for c in list(dict.fromkeys([z_col, "score"])) if c not in group_cols]

    if not columns_to_agg:
        # Fallback if all variables are in grouping columns
        agg_df = df[group_cols].drop_duplicates()
    else:
        agg_df = df.groupby(group_cols)[columns_to_agg].mean().reset_index()
...
def main():
    ...
    # Extract session ID using regex mapping
    session_match = re.search(r"(?P<id>\d{8}_\d{6})", selected_winners_path)
    active_session_id = session_match.group("id") if session_match else "unknown"
    ...
    try:
        params = get_hyperparameters(active_session_id)
        if params:
            t = params.get("TOTAL_TIMESTEPS", "X")
            n = params.get("N_STEPS", "X")
            u = params.get("TOTAL_UPDATES", "X")
            e = params.get("EPOCHS", "X")
            b = params.get("BATCH", "X")
            st.info(f"Configuration Profile: TOTAL_TIMESTEPS = {t} | N_STEPS = {n} | TOTAL_UPDATES = {u} | EPOCHS = {e} | BATCH = {b}")
        else:
            st.text("Session log parameters unavailable")
    except Exception:
        st.text("Session log parameters unavailable")
    ...
    # Dynamic Media Synchronization Layer (Bottom Canvas Placement)
    try:
        image_file = find_convergence_image(active_session_id)
        if image_file:
            st.image(image_file, width="stretch", caption="Session Convergence Progress Metrics")
        else:
            st.text("Convergence graphic asset not populated for this session")
    except Exception:
        st.text("Convergence graphic asset not populated for this session")
